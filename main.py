...
class Spline:
    ...
    def quintic_objective_func(self, params):
        ...
        # curvature cost
        absolute_k = np.absolute(k)
        curvature_cost = 24.0 * np.mean(absolute_k)
    ...
    def optimize_min(self):
        ...
        if result.success:
            ...
            self.opimized_path = Path(x, y, yaw, k)
    ...
    def optimize_basinhopping(self):
        initial_guess = [0, 0, 0, 0, 0]
        bnds = ((-self.bound, self.bound), (-self.bound, self.bound), (-self.bound, self.bound), (-self.bound, self.bound), (-self.bound, self.bound))
...
def main():
    ...
    plt.plot(spline.opimized_path.x, spline.opimized_path.y, '-m', label="optimized")
    ...
    plt.plot([np.rad2deg(iyaw) for iyaw in spline.opimized_path.yaw], "-m", label="optimized")
    plt.grid(True)
    plt.legend()
    ...
    plt.plot(spline.opimized_path.k, "-m", label="optimized")
    plt.grid(True)
    plt.legend()
    plt.xlabel("line length[m]")
